...
public class Graph {
    ...
    public void addEdge(int u, int v) {
        ...
        if (!isConnected(u, v)) {
            ...
            adjacencyList[uIndex].add(v);
            adjacencyList[vIndex].add(u);
        } else {
    ...
    public boolean isConnected(int u, int v) {
        ...
        return adjacencyList[uIndex].contains(v) || adjacencyList[vIndex].contains(u);
    }
    ...
    public void Explore(Vertex v) {
        ...
        int vIndex = Arrays.asList(vertexList).indexOf(v);
        // Traverse all the list check if anyone are already visited or not
        // If no, then explore the vertex
        Iterator<Integer> it = adjacencyList[vIndex].iterator();
        while (it.hasNext()) {
            int key = it.next();
            int vIndex_2 = 0;
            for (int i = 0; i < vertexList.length; i++) {
                if (vertexList[i] == null)
                    continue;
                if (vertexList[i].intKey == key)
                    vIndex_2 = i;
            }
            if (!vertexList[vIndex_2].visited)
                Explore(vertexList[vIndex_2]);
        }
